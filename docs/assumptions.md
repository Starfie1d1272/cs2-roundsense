...
- **C8** [corpus-observed] `time_ran_out` 输方：**存活 T 无败方奖金**（专项审计 261 回合 / 1305 T 行，零反例；死亡 T 拿正常表值奖金）。曾误报的"存活者 +1400"经查为死亡 T 的正常奖金。
...
- **D2** 316 场 / 60325 样本：~90% diff=0、100% 整数残差；剩余 ±200/±500/±300 = buytime 盲区（4/1595）+ 武器映射边缘 + loss-counter cap 边界（cap 未定）。
