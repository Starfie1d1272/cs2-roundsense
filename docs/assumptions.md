...
- **C5** [数据域限制] 316 场语料（职业 demo / ESL 科隆 / 完美世界赛事服务器）的整数差分 + replay 逐帧现金均未检出 CT 团队奖励。Steam 官方 2025-07-16 补丁确认规则存在（"CTs now receive a shared team award for each T eliminated"）。**全部语料来自第三方服务器（自定义经济），无 Valve 官方竞技/Premier 匹配 demo** → 规则是否在官方匹配生效**待官方匹配 demo 验证**（用户 Windows 端打一局官方竞技存 demo 即可）。advisor 输入保留 `ctTeamKillsOnTs`，规则值待定（按用户规则 $50/每名 CT）。
