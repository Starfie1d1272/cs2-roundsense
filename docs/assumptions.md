...
- **C1** [已证实-来源] 竞技/Premier 回合结束奖励：淘汰获胜 $3250、时间获胜(CT) $3250、拆包获胜(CT) $3500、爆炸获胜(T) $3500。来源：cs.fandom Money 页 rev 186480（2026-07-08），访问 2026-08-06。
- **C2** [已证实-来源] 连败奖励：lossStreak 0→$1400、1→$1900、2→$2400、3→$2900、4+→$3400；连败计数"每败 +1、每胜 -1（min 0，max 4）"（2025 经济更新后规则；页面同时保留旧"两连胜后首败 $1500"规则，视为已被取代，代码不实现）。来源同上。
- **C3** [待语料核验] T 下包后失败额外奖金：fandom 标注 CS2 = $600（CS:GO = $800）。**数值存疑**，须用语料 `player-economies.json` 的 startMoney 差分核验。
- **C4** [已证实-来源] 击杀奖励（竞技）：刀 $1500、手枪 $300、CZ $300、SMG $600、P90 $300、霰弹 $900、XM1014 $600、步枪/机枪 $300、AWP $100、手雷 $300、Zeus $100。来源：cs.fandom Money 页（同上）。
- **C5** [代码暂定] 竞技/Premier **无 CT 团队击杀奖励**（fandom 表格中 "Eliminating an enemy (CT) $50" 仅 Casual 列有值）。输入模型保留 `ctTeamKillsOnTs` 字段，规则文件 `ctTeamKillReward: 0`，便于规则更新而非改代码。待语料核验。
...
- **C10** [代码暂定] 首局（手枪局）连败计数从 0 开始；手枪局失败按 lossStreak=0 的 $1400 + 下包奖金处理（fandom 页面另列 "Losing pistol round $1900" 为旧规则，代码采用连败表 + 注释说明）。[待语料核验]
