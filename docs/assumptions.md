...
- **C2** [已证实-语料] 连败奖励：lossStreak 0→$1400、1→$1900、2→$2400、3→$2900、4+→$3400。**计数规则（2026-08-06 整数对账证实，暴力搜索最优组合）**：每败 +1（手枪局 r1/r13 输 +2，奖金 1900 特殊）、每胜 −1（不清零，min 0，max 4）、**time_ran_out（时间耗尽）赢 −2**、每半场（r13 起）重置。索引 = 本回合前连败数。来源：fandom（同上）+ 316 场/61344 样本整数对账（QF1-m1 单场 93.7% 精确归零）。另证实：**手枪局失败奖金 $1900**（Δ≈+55，n=3145）。
- **C3** [已证实-语料] T 下包后失败奖金 **$600**（全 T 队；fandom CS2 值正确，CS:GO 的 $800 排除）。整数对账 plantBonusT Δ=−12（n=4188）。
- **C4** [已证实-语料] 击杀奖励（含 `m4a1`=M4A4 映射修复）：手枪 300、SMG 600、步枪 300、AWP 100——整数对账全部精确；≤2026-03 语料手枪 ≈$25 为早期导出质量异常（cs2df 3.0.0），近期精确 300。
- **C5** [已证实-语料] **CT 共享团队奖励 = 每名 CT 每消灭一名 T +$50**（"消灭"按 victim 口径，含 C4 爆炸/世界击杀；回合末结算，计入下回合 startMoney）。Steam 官方 2025-07-16 补丁引入；2026-08-06 整数对账证实：win ctKills=5 残差归零（决赛场逐样本精确 0），replay 回合末跳变 250/150/50=50×消灭数。**316 场全部生效（含科隆 1/4 决赛与完美世界赛事）——此前"QF 无规则"结论是分析脚本 bug（漏建模 + 半场边界错误），已修正。** advisor 已接线（`ctTeamKillsOnTs` → 投影 +$50/消灭/每人）。
