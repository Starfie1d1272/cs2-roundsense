...
- `scripts/validate-corpus.ts` — **唯一语料验证入口**：
  ...
  - **L4** replay-native cash-transition ledger（`packages/demo-oracle/src/replay-ledger.ts`）：严格归因；科隆 202 场 93,506 transitions：95.2% 精确 + 4.6% 窗口交易 + 0.02% 未解释
...
最终审计数字（2026-08-07）：科隆 202 replay 场 L1 90.6% / L4 严格 95.2% 精确归因；
